...
def debtors_list():
    ...
    if status_filter == 'overdue':
        ...
        query = query.filter(Debtor.due_date <= date.today() + timedelta(days=3), Debtor.due_date >= date.today(), Debtor.balance > 0)
    elif status_filter == 'pending':
        query = query.filter(Debtor.due_date >= date.today(), Debtor.balance > 0)
    if search:
        query = query.filter(db.or_(Debtor.customer_name.ilike(f'%{search}%'), Debtor.customer_phone.ilike(f'%{search}%')))
    ...
    overdue_count = Debtor.query.filter(Debtor.due_date < date.today(), Debtor.balance > 0).count()
    return render_template('debtors_list.html', debtors=debtors, total_debt=total_debt, total_debtors=total_debtors, overdue_count=overdue_count, status_filter=status_filter, search=search, today=date.today())

@debtors_bp.route('/<int:debtor_id>')
@login_required
def debtor_profile(debtor_id):
    debtor = Debtor.query.get_or_404(debtor_id)
    payments = Payment.query.filter_by(debtor_id=debtor_id).order_by(Payment.payment_date.desc()).all()
    sale = Sale.query.get(debtor.sale_id) if debtor.sale_id else None
    sale_items = SaleItem.query.filter_by(sale_id=sale.id).all() if sale else []
    debtor.status_info = get_debt_status(debtor.balance, debtor.due_date)
    return render_template('debtor_profile.html', debtor=debtor, payments=payments, sale=sale, sale_items=sale_items, today=date.today(), now=datetime.now())
...
def add_payment(debtor_id):
    ...
    if amount > debtor.balance:
        ...
        return redirect(url_for('debtors.debtor_profile', debtor_id=debtor_id))
    try:
        payment = Payment(debtor_id=debtor.id, amount=amount, payment_method=payment_method, notes=notes, user_id=current_user.id)
        ...
        db.session.commit()
        log_activity('debt_payment', f"Payment of {amount:,.2f} recorded for {debtor.customer_name}, balance now {debtor.balance:,.2f}", 'debtor', debtor.id)
...
def write_off_debt(debtor_id):
    ...
    try:
        payment = Payment(debtor_id=debtor.id, amount=debtor.balance, payment_method='write_off', notes=f"WRITE OFF - Reason: {reason}", user_id=current_user.id)
        db.session.add(payment)
        ...
        debtor.updated_at = datetime.utcnow()
        db.session.commit()
        log_activity('write_off', f"Debt of {debtor.customer_name} (KSh {debtor.amount_paid:,.2f}) written off. Reason: {reason}", 'debtor', debtor.id)
...
def statement(debtor_id):
    debtor = Debtor.query.get_or_404(debtor_id)
    payments = Payment.query.filter_by(debtor_id=debtor_id).order_by(Payment.payment_date).all()
    sale = Sale.query.get(debtor.sale_id) if debtor.sale_id else None
    sale_items = SaleItem.query.filter_by(sale_id=sale.id).all() if sale else []
    return render_template('debtor_statement.html', debtor=debtor, payments=payments, sale=sale, sale_items=sale_items, today=date.today(), now=datetime.now())
...
def api_summary():
    total_debt = db.session.query(func.sum(Debtor.balance)).scalar() or 0
    overdue_count = Debtor.query.filter(Debtor.due_date < date.today(), Debtor.balance > 0).count()
    return jsonify({'total_debt': total_debt, 'overdue_count': overdue_count, 'debtor_count': Debtor.query.filter(Debtor.balance > 0).count()})
