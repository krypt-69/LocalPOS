...
def add_stock():
    ...
    try:
        ...
        if product:
            ...
            db.session.flush()
            log_activity('stock_add', f"Added {quantity} {product_name}, new stock: {product.current_stock}", 'product', product.id)
            ...
            db.session.flush()
            log_activity('stock_add', f"Created new product {product_name} with {quantity} units", 'product', product.id)
...
def search_product():
    ...
    results = [{'id': p.id, 'name': p.name, 'buying_price': p.buying_price, 'selling_price': p.selling_price, 'category_id': p.category_id, 'current_stock': p.current_stock} for p in products]
...
def get_product(product_id):
    ...
    return jsonify({'id': product.id, 'name': product.name, 'buying_price': product.buying_price, 'selling_price': product.selling_price, 'category_id': product.category_id, 'supplier': product.supplier})
    ...
    try:
        ...
        for item in products:
            ...
            if product:
                ...
                product.selling_price = selling_price
                db.session.flush()
                log_activity('batch_stock_add', f"Added {quantity} to {product_name}", 'product', product.id)
            else:
                product = Product(name=product_name, category_id=int(category_id), current_stock=quantity, buying_price=buying_price, selling_price=selling_price)
                db.session.add(product)
                db.session.flush()
                log_activity('batch_stock_add', f"Created {product_name} with {quantity} units", 'product', product.id)
            stock_entry = StockEntry(product_id=product.id, quantity=quantity, buying_price=buying_price, selling_price=selling_price, supplier=supplier, user_id=current_user.id)
            db.session.add(stock_entry)
            added_count += 1
        db.session.commit()
...
def preview_import():
    ...
    file.save(temp_path)
    return render_template('stock_in_import.html', preview=preview_data, temp_file=temp_path, filename=filename)
...
def confirm_import():
    ...
    for product_data in result['products']:
        try:
            ...
            if product:
                ...
                product = Product(name=product_name, category_id=category.id, current_stock=quantity, buying_price=buying_price, selling_price=selling_price)
                db.session.add(product)
                db.session.flush()
            stock_entry = StockEntry(product_id=product.id, quantity=quantity, buying_price=buying_price, selling_price=selling_price, supplier=supplier, user_id=current_user.id)
            db.session.add(stock_entry)
    ...
    db.session.commit()
    os.remove(temp_file)
    log_activity('import_stock', f"Imported {added_count} products via CSV/Excel", 'stock', None)
